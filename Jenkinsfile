pipeline {
    ...
    stages {
        stage('Checkout') {
            steps {
                echo 'Git Checkout'
                checkout scmGit(branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[credentialsId: 'Jenkins_github', url: 'https://github.com/Devrajnarayanan/jenkins_testing_repo.git']])
                sh 'ls -lrth'
                sh 'git branch'
